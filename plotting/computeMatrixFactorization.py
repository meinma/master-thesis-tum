...
def solve(inpath, outpath, mode):
    ...
    @param mode: Determines the type of approximation ('mf', 'svd' or 'soft')
    ...
    if mode == 'mf':
        ...
        solver = IterativeSVD(convergence_threshold=0.001)
    else:
        solver = SoftImpute(convergence_threshold=0.01)
